...
export function extractSnippet(source, reference, stepFile) {
  ...
  if (fn) {
    ...
    let attachment = parent?.type === 'ExportNamedDeclaration' ? parent.start : start
    ...
    for (const line of lines) {
      const opening = line.match(/^\s*\/\/\s*#region\s+(.+?)\s*$/)
      const closing = /^\s*\/\/\s*#endregion(?:\s+.*)?$/.test(line)
      if (opening) stack.push({ name: opening[1], start: offset + line.length + (source[offset + line.length] === '\r' ? 2 : 1) })
      if (closing) {
        const opened = stack.pop()
        if (!opened) fail('unmatched #endregion')
  ...
  const code = dedent(lines.filter((line) => !/^\s*\/\/\s*#(?:end)?region\b/.test(line)).join('\n'))
...
export async function extractAll(root) {
  ...
  for (const filename of await stepFiles(path.join(root, 'content/lessons'))) {
    ...
    try {
      ...
      const source = await readFile(path.resolve(root, frontmatter.code.file), 'utf8')
      snippets[id] = extractSnippet(source, frontmatter.code, stepFile)
    } catch (error) {
