...
export const COOKIE_NAME = 'worldbuilding-guidebook_tutor'
...
export function getRateLimiters() {
  if (!limiters) {
    ...
    limiters = Object.fromEntries(Object.entries(RATE_RULES).map(([key, [count, duration]]) => [key, new Ratelimit({ redis, prefix: `worldbuilding-guidebook:${key}`, limiter: Ratelimit.fixedWindow(count, duration), timeout: 3000, ephemeralCache: false })]))
  }
  return limiters
}
